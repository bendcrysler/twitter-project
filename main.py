...
def insert_tweet_set(tweet_set, cursor):
    for tweet in tweet_set:
        ...
        if hasattr(tweet, 'reply_to_status_id'):
            #TODO: Make sure in reply to tweet gets added to table to satisfy reference requirement
...
# function that inputs a single tweet into DB, returns none. Used to satisfy referential DB requirements
def insert_single_tweet(tweet, cursor):
    ...
    mentions = tweet.entities['user_mentions']
    if hasattr(tweet, 'retweeted_status'):
        print("ERROR: a retweet has a retweet. this probably can't actually happen.")
        retweeted_tweet = tweet.retweeted_status.id_str
        insert_single_tweet(tweet.retweeted_status, cursor)
    else:
        retweeted_tweet = None
...
home_timeline = api.home_timeline(count=200)
...
# select all tweets
curs.execute("SELECT id FROM tweets")
